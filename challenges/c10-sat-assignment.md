...
```
    ...
    ## 1 corr   0.693     0.781  0.850   0.05 percentile
...
- By itself, how well does `both_SAT` predict `univ_GPA`?
  - Somewhat well; this linear model fitting created an adjusted R^2
    value of 0.5383881, which is certainly more than 0 (indicating there
    is *some* indicative correlation). But because 0.5383881 is also
    quite far from a perfect 1, `both_SAT` is not a very accurate
...
- How well do these models perform, compared to the one you built in q6?
  ...
  - The model with just `high_GPA` has an adjusted R^2 value of
    0.6383604. The model with both variables has an adjusted R^2 value
    of 0.6757461. Comparing these numbers with the 0.5383881 from
    before, both of these models have a much higher correlation than the
    model in q6.
...
- How do the hypothesis test results compare with the results in q6?
  - Both the model with just `high_GPA` and the model with just
    `both_SAT` have a confidence interval that doesn’t include 0, but
    the `high_GPA` model has an interval of 0.5036122 - 0.8296483, which
    is much higher above 0 than `both_SAT`’s interval of 0.001715376 -
    0.003415381.
  - For this model with both `high_GPA` and `both_SAT`, the confidence
    interval on the coefficient of `both_SAT` includes 0, but the model
    in q6 has an interval that doesn’t include 0.
