...
- It appears that the difference lies in rounding the mean velocities.
  Perhaps because the original measurements Michelson took were not
  entirely accurate to begin with (due to his mechanical measuring
  system and because he didn’t measure in a vacuum), he felt that
  rounding his calculations to 5 sig figs would indicate this issue. R
  doesn’t have this sig fig intuition built in, so the table I’ve
  generated includes 7 sig figs.
...
- Michelson predicted an approximate error of 51, which is less than the
  true error since his value and the modern value differ by 151

Make a quantitative comparison between Michelson’s uncertainty and his
error. - (Your response here)

- His uncertainty was approximately 100 km/s off from his error.
...
- The data is scattered with the same general range

- The mean line for the data falls outside the “acceptable” margins
  (dotted horizontal lines) at some points

- The relative peaks and valleys of the two graphs fall on very similar
  days

Differences:

- The real data has a bigger range than the simulated by ~100 km/s. The
  minimum of the real data is 50 km/s less than the simulated data
  (299700 vs 299750). The maximum of the real data is 50 km/s more than
  the simulated data (300150 vs 300100)

- The real data has a mean line that falls outside the dotted lines more
  towards the beginning of the timeframe, while the simulated data falls
  outside the dotted lines more towards the end
...
- In the first graph I generated, I was looking to see if there was a
  correlation between the measured velocity (adjusted for a vacuum) and
  the temperature. There initially doesn’t seem to be any clear
  correlation that can be derived, but if I unfocus my eyes I can see a
  slight positive correlation. This indicates that a higher temperature
  can result in a higher velocity measurement.
  - Michelson’s estimated value for the speed of light is higher than
    the modern speed of light, and this temperature difference would
    explain it. He measured in temperatures far higher than 0 Kelvin,
    which is what exists in a true vacuum. Therefore, his measurements
    were already overestimating the true value. On top of that, he also
    added 92 km/s to his measurements, which is in fact compensating in
    the wrong direction if we follow the positive trend between
    temperature and measured velocity of light.
- In the second graph I generated, I was looking to see if the measured
  velocity changed with the measurement’s distinctness. I used a box
  plot to display this information, since Distinctness is a discrete
  variable and VelocityVacuum is a continuous variable for our purposes.
  The mean velocity increases as the distinctness increases. Since a
  lack of distinctness suggests more inaccurate data, we can see that
  the more unclear the data, the more the measurement tends to
  underestimate the true value of the speed of light. This conclusion
  was drawn from looking at the means of the boxplots.
  - In my interpretation, this trend demonstrated in the second plot
    actually contradicted the fact that Michelson overestimated the
    speed of light. Because a worse distinction implies a lower measured
    speed (according to the plot), I’d intuitively think that
    Michelson’s prediction would be less than the real speed of light.
    However, all values across the three distinctnesses are far above
    the modern speed of light anyways. We already determined from the
    first plot that a higher temperature led to a higher speed of light,
    which accounts for this. So because all the values are already quite
    greater than the expected value, the trend observed doesn’t actually
    help to explain the difference between Michelson’s and the expected
    speed of light.
