...
df_q2 <-
  mpg %>%
  filter(str_detect(string = trans, pattern = "\\d"))
...
df_q3 <-
  mpg %>%
  mutate(tmp = str_extract(string = trans, pattern = "[[:alpha:]]\\d"))
df_q3 %>%
  select(tmp)
```
...
df_q5 <-
  mpg %>%
  mutate(trans = str_remove(string = trans, pattern = "\\(.*\\)"))
df_q5
```
