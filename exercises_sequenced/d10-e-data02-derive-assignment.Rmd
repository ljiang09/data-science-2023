...
-   They produce the same output, just one uses the pipe operator.
    Syntax difference, output no difference
...
-   with just the summarize function, we get just one value for the
    price (the mean?). This is a summary of the entire data table

-   With the addition of the group_by function, we get a subtable of
    diamonds sorted by color. This represents the price summary of all
    combinations of color and clarity
...
-   Comment on the relative values of `cov_depth` and `cov_diff`.
    -   cov_diff is really big, which shows the diffs vary a lot

    -   cov_depth is really small, which shows the depths are pretty
        consistent
-   Why is `cov_diff` so large?
    -   The depths don't include a factor of 100 (as per the note
        above). Diff is based on the depth (not including the factor of
        100) and the calculated depth (which does include this). Because
        of this huge discrepancy, a huge covariance was found.
...
-   fair and premium diamonds sell for the most typically, and ideal
    diamonds typically sell for the least
-   The standard deviation generally increases as quality increases,
    except for ideal diamonds
-   The covariance generally increases as quality increases
