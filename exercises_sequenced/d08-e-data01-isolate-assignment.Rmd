...
```{r q2-task}
df_q2 <- flights %>%
  select(dest, origin, carrier, everything())
df_q2
```
...
-   The error was that it wasn't actually making a comparison, and in
    addition it was attempting to compare to a variable rather than an
    object. To fix it, i added an extra "=" and quotes around LAX

-   This code is trying to get data for flights only regarding LAX
...
-   The longest flights are to HNL, or Honolulu. Hawaii is further than
    the rest of the US, it seems
