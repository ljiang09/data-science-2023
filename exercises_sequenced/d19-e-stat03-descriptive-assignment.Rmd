...
A *statistic* is a numerical summary of a sample. Statistics are useful because they provide a useful summary about our data. A histogram gives us a rich summary of a dataset: for example the departure delay time in the NYC flight data.
...
test_iqr <-
  quantile(v_test_iqr, probs = 0.75, na.rm = TRUE) - quantile(v_test_iqr, probs = 0.25, na.rm = TRUE)
```
...
-   Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
    -   HA has a sd of 74 and IQR of 6. This implies that the data has a lot of outliers
...
-   For what values of `slope` is the correlation positive?
    -   whenever the slope is greater than 0
-   For what values of `slope` is the correlation negative?
    -   whenever the slope is less than 0
...
df_quad %>%
    summarize(rho = cor(x, y, use = "na.or.complete"))
...
-   What do you predict the correlations to be?
    -   I expect the correlation to be around 0 - 0.5 (or much smaller than 1)
