...
-   Line plots by default assume all the points are from the same line.
    This dataset violates this assumption because the points are
    actually from different lines based on car type
...
-   The raw data shows that the overall trend rises and falls every 10
    years or so (it shows information visible on a smaller time scale)

-   The smoothed data shows the overall trend for the entire period of
    time, which is that the unemployment rate has increased over time,
    with an exception in the 1990s.
