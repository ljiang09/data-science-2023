...
### **q3** What are the main differences between `install.packages()` and `library()`? How often do you need to call each one?
    ...
    `install.packages()` installs the packages into the repo. It's like a pip install. It only needs to occur once.

    `library()` calls the package into the file, like an import statement with Python. It needs to happen every time there is a file that relies on the library.
