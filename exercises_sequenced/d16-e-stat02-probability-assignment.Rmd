...
count_total <- nrow(df_z)
count_A <- df_z %>%
  filter(z <= 0) %>%
  nrow()

fr <- count_A / count_total
fr
```
...
-   What value of `fr` did you predict?
    -   I predicted fr to be very close to 0.5, since `df_z` is an approximately normal distribution
-   Did your prediction match your calculation?
    -   Yes, 0.54 is rather close to 0.5
...
-   What value does `fr` appear to be limiting towards?
    -   `fr` appears to be getting close to 0.9516
...
-   What was your `fr` prediction in **q1**? How does `p0` compare?
    -   My prediction in q1 was about 0.5. I expected it to not be exactly 0.5, since I observed that the tibble that was created in the code chunk was rather randomized. Here, `p0` is exactly 0.5.
