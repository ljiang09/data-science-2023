...
df_q1 <-
  genes %>%
  unite(col = "children", parent1, parent2, sep="", remove = FALSE) %>%
  pivot_wider(names_from = parent2, values_from = children)
...
df_q2 <-
  alloys_mod %>%
  pivot_longer(names_to = "type", values_to = "value", cols = c(E00, mu00, E45, mu45)) %>%
  separate(col = type, into = c("letter", "number"), sep=-2, convert = TRUE) %>%
