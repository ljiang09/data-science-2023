...
function's documentation can be *extremely* helpful.
    ...
    Put a ? in front of the function name, and don't put the parentheses
...
```
    ...
    You can filter across multiple criteria using a single logical expression, like so:
    filter(starwars, hair_color == "none" | eye_color == "black")

    Written using pipe operators:
    starwars %>%
...
iris %>%
  as_tibble() %>%
  filter(
    Species == "setosa" & Sepal.Length >= 5.1 & Sepal.Length <= 6.4
  )
```
