...
### **q0** Remember from `e02-data-basics` there were *simple checks* we're supposed to do? Do those simple checks on the diamonds dataset below.
...
### **q1** Run the chunk below, and study the plot. Was hypothesis 1 correct? Why or why not?
...
-   The hypothesis was actually wrong, fair cuts (the worst cut) is far
    less common than ideal diamonds. A likely reason for this is that
    people who cut and sell diamonds want to extract the most value out
    of them due to their rareness, so they would not waste diamonds on
    bad cuts.
...
-   This is not a very useful plot. Because there are so many data
    points, everything overlaps and you cannot actually see clumping of
    data. It just looks like lines. It looks like all cuts are generally
    valued with equal spread.
...
-   It actually does not support hypothesis 2. Ideal cuts are generally
    cheaper than premium, and even Very Good cuts! The mean is lower,
    the 75% quartile is less.
...
-   Yes it could, a bigger diamond equals a higher value. If the cutter
    is shaving off a lot of the diamond, then at some point enough will
    be shaved off that the diamond actually starts losing value (which
    an Ideal cut appears to be that point)
...
-   This graph indicates that people prefer carat over cut. I say this
    because the premium diamonds have higher cut, which tells me that
    people keep diamonds at premium quality to preserve the size.

-   The article supports this: "Today, because of the relative
    importance of carat weight among buyers, many diamonds are often
    intentionally cut poorly to increase carat weight"
