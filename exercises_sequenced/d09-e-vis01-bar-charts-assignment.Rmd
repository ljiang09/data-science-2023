...
### **q2** The following code attempts to visualize `cty, hwy` from `mpg` usin `geom_col()`. There's something fishy about the `hwy` values; what's wrong here?
...
-   `dodge` makes the plot look more correlated

-   This is kind of a weird way to represent the datapoints
...
-   The graph counts the number of each type of car, and color codes it.
    Pretty basic but very easy to read
...
-   This displays another variable and maximizes the usage of color.
    Rather than telling the same thing using both the x-axis and the
    color, the x axis distinguishes between car classes and the color is
    used to determine the drv.
...
-   What was the difference between `v1` and `v2`?
    -   V2 included a secondary variable: drv. More variables = more
        observations
...
-   some companies only make specific cars. Hondas only make
    subcompacts, jeeps/land rovers/etc only make SUV's, and such. Toyota
    has the most variance with 4 types of cars
