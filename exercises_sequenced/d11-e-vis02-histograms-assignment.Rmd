...
```{r q2-task}
mpg %>%
  ggplot(aes(cty, color = class), scales = "free_y") +
  geom_density()
```
...
-   At 0.01 and lower, a clear repeating trend occurs where there are a lot of exact "round numbers" for carats (ie. 0.3, 0.4, 0.5, 0.7), and this curves off until the next round number

-   This curving-off trend is not seen when bin width is 0.05, but you can kind of tell there are peaks in specific carat sizes

-   At bin width of 0.1, you can't really see much useful information
